...
$query1='SELECT product_id, tocken_id, product_name, description, art_price,gif, image,type,start_date, end_date,creator_id, owner_id, uri, txHash, auction FROM nfts WHERE product_id = :id';
 ...
 $id = htmlspecialchars(strip_tags($id));
    ...
    $stmt->bindParam(':id', $id);
        ...
        $query = 'INSERT INTO featureaudios (product_id, tocken_id, product_name, description, art_price,gif, image,type,start_date, end_date,creator_id, owner_id, uri, txHash, auction,sequence) values(:product_id,:tocken_id, :product_name, :description, :art_price, :gif, :image, :type, :start_date, :end_date, :creator_id, :owner_id, :uri, :txHash, :auction, :sequence)
...
private function countRows(){
        ...
        $query = 'SELECT MAX(sequence) as sequence FROM featureaudios';
        $stmt = $this->conn->prepare($query);
         $stmt->execute();
        $row=$stmt->fetch(PDO::FETCH_ASSOC);
        return $row['sequence'];
